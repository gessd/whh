...
{
    m_pFunc->nResult = initService();
    if(0 != m_pFunc->nResult){
        return;
    }
...
{
    if(NULL == m_pFunc->pUserEx) return;
    m_pFunc->nResult = addUser(m_pFunc->pUserEx);
}

void ContorllThread::thread_userDel()
{
    if(NULL == m_pFunc->pUserEx) return;
    m_pFunc->nResult = delUser(m_pFunc->pUserEx);
}

void ContorllThread::thread_veinAdd()
{
    if(NULL == m_pFunc->pVeinEx) return;
    qDebug()<<"---- veinAdd "<<m_pFunc->pVeinEx->user_id<<m_pFunc->pVeinEx->fg_id<<m_pFunc->pVeinEx->temp;
    m_pFunc->nResult = addVein(m_pFunc->pVeinEx);
}

void ContorllThread::thread_veinDel()
{
    if(NULL == m_pFunc->pVeinEx) return;
    m_pFunc->nResult = delVein(m_pFunc->pVeinEx);
}
...
void ContorllThread::thread_getVeinChara()
{
    ...
    m_pFunc->lResult = getVeinChara(m_pFunc->pChara1);
    qDebug()<<"---- getVeinChara:"<<m_pFunc->lResult;
#ifdef _InterfaceDebug_
    sleep(2);
    m_pFunc->pChara1 = {"aaa"};;
    m_pFunc->fResult = 111;
...
void ContorllThread::thread_createVeinTemp()
{
    ...
    m_pFunc->lResult = createVeinTemp(m_pFunc->pChara1, m_pFunc->pChara2, m_pFunc->pChara3, m_pFunc->pTemp);
}
